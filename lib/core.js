...
class ThermalPrinter {
  ...
  italic(enabled) {
    if (enabled) this.append(this.printer.config.TXT_ITALIC_ON);
    else this.append(this.printer.config.TXT_ITALIC_OFF);
  }
  ...
  tableCustom(data, newLine = true) {
    const totalWidth = this.config.width;
    let remainingWidth = totalWidth;
    const secondLine = [];
    let secondLineEnabled = false;

    // Precompute cell widths
    let cellWidths = data.map((obj) => {
      if (obj.width) {
        const width = Math.floor(totalWidth * obj.width);
        remainingWidth -= width;
        return width;
      } else if (obj.cols) {
        remainingWidth -= obj.cols;
        return obj.cols;
      } else {
        // Default width for evenly distributed columns
        const width = Math.floor(totalWidth / data.length);
        remainingWidth -= width;
        return width;
      }
    });

    // Distribute the remaining space across columns
    for (let i = 0; i < remainingWidth; i++) {
      cellWidths[cellWidths.length - 1 - (i % cellWidths.length)] += 1;
    }
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (obj.align === 'CENTER') {
        const spaces = Math.floor((cellWidth - obj.text.length) / 2);
        this.append(Buffer.from(' '.repeat(spaces)));
        this.append(obj.text);
        this.append(Buffer.from(' '.repeat(cellWidth - obj.text.length - spaces)));
      } else if (obj.align === 'RIGHT') {
        const spaces = cellWidth - obj.text.length;
        this.append(Buffer.from(' '.repeat(spaces)));
        this.append(obj.text);
      } else {
        this.append(obj.text);
      ...
      // Handle overflow text for the next line
      if (tooLong) {
        secondLineEnabled = true;
        obj.text = obj.originalText.substring(cellWidth - 1).trimStart(); // Trim leading spaces
        secondLine.push({ ...obj }); // Push overflow text to second line
      } else {
        secondLine.push({ text: '', ...obj }); // Empty text for second line
      }
    }

    this.newLine();

    // Add a newline after processing the row if `newLine` is true
    if (newLine) {
      this.newLine();
    }
    ...
    if (secondLineEnabled) {
      this.tableCustom(secondLine, newLine);
    }
  }
  ...
  */
  _fold(text, lineSize, breakWord, lineArray = []) {
    ...
    if (text.length <= lineSize) {
      lineArray.push(text); // If the remaining text fits, add it as-is.
      return lineArray;
    }
    ...
    if (!breakWord) {
      // Attempt to break at the last whitespace in the line.
      const lastSpaceIdx = line.lastIndexOf(' ');
      if (lastSpaceIdx !== -1) {
        line = text.substring(0, lastSpaceIdx); // Break at the last space.
        lineArray.push(line.trim()); // Trim extra spaces before adding.
        return this._fold(text.substring(lastSpaceIdx + 1), lineSize, breakWord, lineArray);
      }
    }

    // If no whitespace is found or breaking anywhere is allowed, break at `lineSize`.
